...
// Same-origin by default: the API and the site are served by one process.
// In dev, Vite proxies /api to the local API server (see vite.config.ts).
...
export interface ActiveSeasonPlayer {
    id: string;
    name: string;
    current_elo: number;
    is_active: boolean;
...
export const seasonsApi = {
    ...
    async getActiveSeasonPlayers(): Promise<ActiveSeasonPlayer[]> {
        return apiCall<ActiveSeasonPlayer[]>('/api/seasons/active/players', {
            method: 'GET',
        });
    },
};
